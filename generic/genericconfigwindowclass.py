...
# helper class to generate config
class GenericCheckboxConfig:
...
class GenericTextConfig:
    def __init__(self, init="", maxLength=64):
        self.init = init
    ...
    def GetConfig(self):
        ...
        c["type"] = "text"
        c["init"] = self.init
        c["maxLength"] = self.maxLength
        
        return c

class GenericIntConfig:
    def __init__(self, init=0, limitMin = -1, limitMax = 1):
        self.init = init
...
class GenericIPConfig:
    def __init__(self, init="127.0.0.1"):
        self.init = init
...
class GenericConfigurationWindow(tk.Toplevel):
    def __init__(self, parent, config, onSaveCallback):
        ...
        for e in self.config:
            ...
            if c["type"] == "text":
                ...
                self._GenerateTextEntry(e, c["init"])
            elif c["type"] == "dropdown":
                self._GenerateDropDownEntry(e, c["init"], c["choices"])
            elif c["type"] == "checkbox":
                self._GenerateCheckboxEntry(e, c["init"])
    ...
    def OnButtonSave(self) -> None:
        ...
        for e in self.userInputs:
            ...
            if c["type"] == "text":
                ...
                self._SaveDropDownVariable(key, var)
            elif c["type"] == "checkbox":
                self._SaveCheckboxVariable(key, var)
    ...
    def _GenerateTextEntry(self, key, initValue) -> None:
        ...
        tk.Entry(self.inputFrame, textvariable=self.userInputs[row][1]).grid(row=row, column=1, sticky=tk.EW, padx=5, pady=5)
        
        return
    
    def _GenerateDropDownEntry(self, key, initValue, choices) -> None:
        # generate a label to present the variables name
        name = str(key) + " :"
        row = len(self.userInputs)
        ...
        combobox = ttk.Combobox(self.inputFrame, state="readonly", values=choices)
        combobox.set(initValue)
        combobox.grid(row=row, column=1, sticky=tk.EW, padx=5, pady=5)
        self.userInputs.append([key, combobox])
        
        return
    
    def _GenerateCheckboxEntry(self, key, initValue) -> None:
        # generate a label to presetn the variable name
        name = str(key) + " :"
        row = len(self.userInputs)
        ...
        # and the checkbox
        var = tk.IntVar()
        checkbox = tk.Checkbutton(self.inputFrame, variable=var)
        checkbox.grid(row=row, column=1, sticky=tk.W)
        
        if initValue:
            checkbox.select()
        
        self.userInputs.append([key, var])
    ...
    def _SaveDropDownVariable(self, key, var):
        # nothing to check here, just store the variables
        self.userInputValues[key] = var.get()
        
        return
    
    def _SaveCheckboxVariable(self, key, var):
        # nothing to check
        self.userInputValues[key] = var.get()
